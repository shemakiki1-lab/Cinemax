...
adminRouter.get("/api/admin/gens-access", (_req, res) => {
  const list = [...db.data.gens_access]
    .sort((a, b) => (a.last_accessed_at < b.last_accessed_at ? 1 : -1))
    .map((g) => {
      const u = getUserById(g.user_id);
      return {
        userId: g.user_id,
        name: u?.name || g.user_name,
        email: u?.email || g.user_email,
        status: u?.status || "unknown",
        firstAccessedAt: g.first_accessed_at,
        lastAccessedAt: g.last_accessed_at,
