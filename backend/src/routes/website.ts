...
authRouter.post("/api/auth/forgot-password", async (req, res) => {
  ...
  // Product decision: after the user has typed the email they registered
  // with, prompt them for a new password immediately — no round trip through
  // the inbox. We still fire the reset-email in the background when the
  // mailer is configured, so users have a record, but the API always returns
  // the resetToken so the frontend can jump straight to the "set new
  // password" screen with the token pre-filled.
  if (isMailerConfigured()) {
    const baseUrl = `${req.protocol}://${req.get("host")}`;
    sendPasswordResetEmail(normalized, token, baseUrl).catch((err) => {
      console.error("[auth] Failed to send password reset (background):", err);
    });
  }
  res.json({
    ok: true,
    message: "Account verified. Set your new password below.",
    resetToken: token,
  });
});
...
authRouter.get("/api/download-apk", (_req, res) => {
  // Note: Cinemax is a PWA (Progressive Web App) and does not have a native APK.
  // This route is reserved for future native app development.
  // Currently, users should use the browser's native "Install App" flow.
  res.status(501).json({
    error: "Native APK not available. Cinemax is a Progressive Web App (PWA). Use the 'Install App' button to add it to your home screen.",
    pwaInstructions: {
...
authRouter.get("/api/auth/google/callback", async (req, res) => {
  ...
  if (!code || !state || !rawCookie) {
    res.status(400).send("Google sign-in was interrupted. Please try again.");
    return;
  }
  let saved: { state: string; returnTo: string; exp: number } | null = null;
  try {
    saved = JSON.parse(Buffer.from(rawCookie, "base64").toString("utf8"));
  } catch {
    saved = null;
  }
  if (!saved || saved.state !== state || Date.now() > saved.exp) {
    res.status(400).send("Google sign-in session expired. Please try again.");
    return;
  }
  ...
  try {
    ...
    if (!tokenRes.ok) {
      ...
      res.status(502).send("Google didn't accept the sign-in code. Please try again.");
      return;
    }
    const tokenJson: any = await tokenRes.json();
    const accessToken: string = tokenJson.access_token;
    if (!accessToken) {
      res.status(502).send("Google returned no access token. Please try again.");
      return;
    }
    ...
    if (!userRes.ok) {
      res.status(502).send("Couldn't read your Google profile. Please try again.");
      return;
    }
    ...
    if (!email || !emailVerified) {
      res.status(400).send("Your Google account has no verified email. Please try a different account.");
      return;
    }
    ...
    if (user.status === "banned" || user.status === "suspended") {
      res.status(403).send("This account isn't allowed to sign in. Please contact support.");
      return;
    }
    ...
    res.status(500).send("Something went wrong signing you in with Google. Please try again.");
  }
});
