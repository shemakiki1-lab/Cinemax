...
authRouter.post("/api/stream/resolve", async (req, res) => {
  ...
  try {
    ...
    // naive search for obvious media links; prefers mp4 then m3u8
    const mp4Match = text.match(/https?:\/\/[^\"'<>\s]+\.mp4/gi);
    if (mp4Match && mp4Match.length > 0) return res.json({ ok: true, sourceUrl: mp4Match[0] });
    const hlsMatch = text.match(/https?:\/\/[^\"'<>\s]+\.m3u8/gi);
    if (hlsMatch && hlsMatch.length > 0) return res.json({ ok: true, sourceUrl: hlsMatch[0] });
...
authRouter.get("/api/proxy", async (req, res) => {
  ...
  try {
    ...
    const allowedHosts = ["vidsrc.xyz", "embed.su", "vidlink.pro", "cdn.jsdelivr.net", "storage.googleapis.com"];
