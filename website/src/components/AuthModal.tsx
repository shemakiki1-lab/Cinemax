...
type SignupStep = "form" | "verify" | "onboarding";
...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const handleSignupVerifySubmit = async (e: React.FormEvent) => {
    ...
    // Show onboarding after successful verification
    setSignupStep("onboarding");
  };

  const handleOnboardingComplete = async (preferences: UserOnboardingData) => {
    try {
      const response = await fetch(`${process.env.VITE_API_BASE || ''}/api/auth/onboarding`, {
        method: 'POST',
  ...
  /** Step 1 of forgot flow — verify the email is registered and immediately prompt for new password */
  const handleForgotEmailSubmit = async (e: React.FormEvent) => {
    ...
    // Auto-fill the reset token if provided, otherwise user will need to enter it
    if (reset.resetToken) {
      setOtp(reset.resetToken);
    }
    setInfo("Account found. Please set your new password below.");
    setForgotStep("reset");
    setPassword("");
    setConfirmPassword("");
  };
