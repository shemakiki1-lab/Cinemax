...
export const InstallAppButton: React.FC<{ variant?: "sidebar" | "card" | "header"; label?: string }> = ({
  ...
  // -------------------------------------------------------------------------
  // HEADER VARIANT — always renders an anchor that downloads /cinemax.apk.
  // Using a real <a href download> means the browser fires the native "Save
  // File" dialog with the correct filename, whether the .apk file is served
  // by the frontend host, a CDN, or a proxy — no JS needed.
  // -------------------------------------------------------------------------
  if (variant === "header") {
    return (
      <a
