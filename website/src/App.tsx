...
const CinemaxDashboard: React.FC = () => {
  ...
  // Genre ID mapping for onboarding preferences
  ...
  // Personalized sections based on user onboarding preferences
  const personalizedSections = currentUser?.onboarding?.favoriteGenres
    ? currentUser.onboarding.favoriteGenres.map((genre) => ({
        id: `personalized_${genre}`,
        label: `Your Favorites: ${genre.charAt(0).toUpperCase() + genre.slice(1)}`,
        genreId: genreIdMap[genre.toLowerCase()],
        visible: true,
      }))
    : [];
  ...
  if (personalizedSections.length > 0) {
    personalizedSections.forEach((section) => {
      homepageSectionData[section.id] = {
        movies: [], // Will be populated dynamically when needed
