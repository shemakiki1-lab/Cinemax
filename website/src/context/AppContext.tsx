...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const maybeShowAdminDestination = (profile: UserProfile) => {
    if (profile.role === "admin") {
      // Auto-hand-off to the standalone admin panel as soon as an admin
      // signs in. Falls back to opening the chooser modal if the portal
      // URL request fails, so the admin still has a way in.
      goToAdminPanel().catch(() => setAdminDestinationOpen(true));
    }
  };
